...
public class UIManager : MonoBehaviour
{
    ...
    // Whether it's the start of the game (WG)
    [Tooltip("Whether the start screen on the level should be displayed")]
    [SerializeField] private bool displayStartScreen = false;
    ...
    public void TogglePause()
    {
        if (allowPause)
        {
            ...
            {
                // Check if it's the start of the game (WG)
                if (gameStart == true)
                {
                    // If so, display the start screen
                    GoToPageByName("StartScreen");
                    gameStart = false;
                }
    ...
    private void Start()
    {
        ...
        InitilizeFirstPage();
        UpdateUI();
        // Make sure the start screen is displayed (WG)
